#[derive(Debug, Eq, PartialEq)]
struct Struct(i32);

trait DummyTrait {}
impl DummyTrait for i32 {}

#[moq::automock]
trait Trait<Gen1, Gen2>
where
    Gen2: DummyTrait,
{
    fn method(&self, arg: Gen1);
    fn method_no_args(&self);
    fn method_lifetime<'a>(&'a self, arg: &'a Gen2);
    fn method_multiple_args<'a, 'b, 'c>(
        &self,
        arg1: i32,
        arg2: String,
        arg3: Struct,
        arg4: &'a Gen1,
        arg5: &'b Gen2,
        arg6: &'c i32,
    );

    fn method_def(&self, arg: Gen1) {
        self.method(arg)
    }
    fn method_no_args_def(&self) {
        self.method_no_args()
    }
    fn method_lifetime_def<'a>(&'a self, arg: &'a Gen2) {
        self.method_lifetime(arg)
    }
    fn method_multiple_args_def<'a, 'b, 'c>(
        &self,
        arg1: i32,
        arg2: String,
        arg3: Struct,
        arg4: &'a Gen1,
        arg5: &'b Gen2,
        arg6: &'c i32,
    ) {
        self.method_multiple_args(arg1, arg2, arg3, arg4, arg5, arg6)
    }

    #[moq(use_default)]
    fn method_def_use_default(&self, arg: Gen1) {
        self.method(arg)
    }
    #[moq(use_default)]
    fn method_no_args_def_use_default(&self) {
        self.method_no_args()
    }
    #[moq(use_default)]
    fn method_lifetime_def_use_default<'a>(&'a self, arg: &'a Gen2) {
        self.method_lifetime(arg)
    }
    #[moq(use_default)]
    fn method_multiple_args_def_use_default<'a, 'b, 'c>(
        &self,
        arg1: i32,
        arg2: String,
        arg3: Struct,
        arg4: &'a Gen1,
        arg5: &'b Gen2,
        arg6: &'c i32,
    ) {
        self.method_multiple_args(arg1, arg2, arg3, arg4, arg5, arg6)
    }

    fn method_ret(&self, arg: i32) -> i32;
    fn method_no_args_ret(&self) -> i32;
    fn method_lifetime_ret<'a>(&'a self, arg: &'a i32) -> &'a i32;
    fn method_multiple_args_ret<'a, 'b, 'c>(
        &self,
        arg1: i32,
        arg2: String,
        arg3: Struct,
        arg4: &'a Gen1,
        arg5: &'b Gen2,
        arg6: &'c i32,
    ) -> (i32, String, Struct, &'a Gen1, &'b Gen2, &'c i32);
}

// In rust you can't specify lifetimes for closure
// and auto lifetimes are wrong
fn method_lifetime_ret_check(arg: &i32) -> &i32 {
    assert_eq!(*arg, 1);
    arg
}

fn method_multiple_args_ret_check<'a, 'b, 'c>(
    arg1: i32,
    arg2: String,
    arg3: Struct,
    arg4: &'a i32,
    arg5: &'b i32,
    arg6: &'c i32,
) -> (i32, String, Struct, &'a i32, &'b i32, &'c i32) {
    assert_eq!(arg1, 1);
    assert_eq!(arg2, "2");
    assert_eq!(arg3, Struct(3));
    assert_eq!(arg4, &4);
    assert_eq!(arg5, &5);
    assert_eq!(arg6, &6);

    (arg1, arg2, arg3, arg4, arg5, arg6)
}

fn make_mock<'a>() -> impl Trait<i32, i32> {
    TraitMock::new()
        .expect_call_method(|arg: i32| {
            assert_eq!(arg, 1);
        })
        .expect_call_method_no_args(|| {
            assert_eq!(true, true);
        })
        .expect_call_method_lifetime(|arg: &i32| {
            assert_eq!(*arg, 1);
        })
        .expect_call_method_multiple_args(
            |arg1: i32, arg2: String, arg3: Struct, arg4: &i32, arg5: &i32, arg6: &i32| {
                assert_eq!(arg1, 1);
                assert_eq!(arg2, "2");
                assert_eq!(arg3, Struct(3));
                assert_eq!(arg4, &4);
                assert_eq!(arg5, &5);
                assert_eq!(arg6, &6);
            },
        )
        .expect_call_method_def(|arg: i32| {
            assert_eq!(arg, 1);
        })
        .expect_call_method_no_args_def(|| {
            assert_eq!(true, true);
        })
        .expect_call_method_lifetime_def(|arg: &i32| {
            assert_eq!(*arg, 1);
        })
        .expect_call_method_multiple_args_def(
            |arg1: i32, arg2: String, arg3: Struct, arg4: &i32, arg5: &i32, arg6: &i32| {
                assert_eq!(arg1, 1);
                assert_eq!(arg2, "2");
                assert_eq!(arg3, Struct(3));
                assert_eq!(arg4, &4);
                assert_eq!(arg5, &5);
                assert_eq!(arg6, &6);
            },
        )
        .expect_call_method(|arg: i32| {
            assert_eq!(arg, 1);
        })
        .expect_call_method_no_args(|| {
            assert_eq!(true, true);
        })
        .expect_call_method_lifetime(|arg: &i32| {
            assert_eq!(*arg, 1);
        })
        .expect_call_method_multiple_args(
            |arg1: i32, arg2: String, arg3: Struct, arg4: &i32, arg5: &i32, arg6: &i32| {
                assert_eq!(arg1, 1);
                assert_eq!(arg2, "2");
                assert_eq!(arg3, Struct(3));
                assert_eq!(arg4, &4);
                assert_eq!(arg5, &5);
                assert_eq!(arg6, &6);
            },
        )
        .expect_call_method_ret(|arg: i32| {
            assert_eq!(arg, 1);
            arg
        })
        .expect_call_method_no_args_ret(|| {
            assert_eq!(true, true);
            1
        })
        .expect_call_method_lifetime_ret(method_lifetime_ret_check)
        .expect_call_method_multiple_args_ret(method_multiple_args_ret_check)
}

#[test]
fn test() {
    call_generic(make_mock());
    call_boxed(Box::new(make_mock()));
    call_ref(&make_mock());
}

fn call_generic<T: Trait<i32, i32>>(obj: T) {
    obj.method(1);
    obj.method_no_args();
    obj.method_lifetime(&1);
    obj.method_multiple_args(1, "2".to_string(), Struct(3), &4, &5, &6);

    obj.method_def(1);
    obj.method_no_args_def();
    obj.method_lifetime_def(&1);
    obj.method_multiple_args_def(1, "2".to_string(), Struct(3), &4, &5, &6);

    obj.method_def_use_default(1);
    obj.method_no_args_def_use_default();
    obj.method_lifetime_def_use_default(&1);
    obj.method_multiple_args_def_use_default(1, "2".to_string(), Struct(3), &4, &5, &6);

    assert_eq!(obj.method_ret(1), 1);
    assert_eq!(obj.method_no_args_ret(), 1);
    assert_eq!(obj.method_lifetime_ret(&1), &1);
    assert_eq!(
        obj.method_multiple_args_ret(1, "2".to_string(), Struct(3), &4, &5, &6),
        (1, "2".to_string(), Struct(3), &4, &5, &6)
    );
}

fn call_boxed(obj: Box<dyn Trait<i32, i32>>) {
    obj.method(1);
    obj.method_no_args();
    obj.method_lifetime(&1);
    obj.method_multiple_args(1, "2".to_string(), Struct(3), &4, &5, &6);

    obj.method_def(1);
    obj.method_no_args_def();
    obj.method_lifetime_def(&1);
    obj.method_multiple_args_def(1, "2".to_string(), Struct(3), &4, &5, &6);

    obj.method_def_use_default(1);
    obj.method_no_args_def_use_default();
    obj.method_lifetime_def_use_default(&1);
    obj.method_multiple_args_def_use_default(1, "2".to_string(), Struct(3), &4, &5, &6);

    assert_eq!(obj.method_ret(1), 1);
    assert_eq!(obj.method_no_args_ret(), 1);
    assert_eq!(obj.method_lifetime_ret(&1), &1);
    assert_eq!(
        obj.method_multiple_args_ret(1, "2".to_string(), Struct(3), &4, &5, &6),
        (1, "2".to_string(), Struct(3), &4, &5, &6)
    );
}

fn call_ref(obj: &dyn Trait<i32, i32>) {
    obj.method(1);
    obj.method_no_args();
    obj.method_lifetime(&1);
    obj.method_multiple_args(1, "2".to_string(), Struct(3), &4, &5, &6);

    obj.method_def(1);
    obj.method_no_args_def();
    obj.method_lifetime_def(&1);
    obj.method_multiple_args_def(1, "2".to_string(), Struct(3), &4, &5, &6);

    obj.method_def_use_default(1);
    obj.method_no_args_def_use_default();
    obj.method_lifetime_def_use_default(&1);
    obj.method_multiple_args_def_use_default(1, "2".to_string(), Struct(3), &4, &5, &6);

    assert_eq!(obj.method_ret(1), 1);
    assert_eq!(obj.method_no_args_ret(), 1);
    assert_eq!(obj.method_lifetime_ret(&1), &1);
    assert_eq!(
        obj.method_multiple_args_ret(1, "2".to_string(), Struct(3), &4, &5, &6),
        (1, "2".to_string(), Struct(3), &4, &5, &6)
    );
}
